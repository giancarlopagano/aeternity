...
handle_chain_set_initial_share_target(State) ->
    ...
    send_set_target_ntf(ShareTarget, State#state{accept_blocks = true,
...
handle_chain_new_block(#{hash := BlockHash, target := BlockTarget,
                       ...
                       #state{phase = authorized, accept_blocks = true,
                              jobs = Jobs} = State) ->
    NewJob = aestratum_job:new(BlockHash, BlockTarget, BlockVersion),
    case aestratum_job_queue:member(NewJob, Jobs) of
        true  -> {no_send, State};  %% Unlikely to happen?
        false -> handle_new_job(NewJob, State)
    end;
...
handle_new_job(NewJob, #state{share_target = ShareTarget, jobs = Jobs} = State) ->
    ...
    NewJobShareTarget =
        case aestratum_job_queue:share_target(NewJob, Jobs) of
            {ok, NewShareTarget}     -> NewShareTarget;
            {error, not_enough_jobs} -> ShareTarget
        end,
    ShareTargetDiffThreshold = ?SHARE_TARGET_DIFF_THRESHOLD,
    case aestratum_target:diff(NewJobShareTarget, OldJobShareTarget) of
        {_Change, Percent} when Percent > ShareTargetDiffThreshold ->
            %% We need to send set_target and then notify notifications.
            NewJob1 = aestratum_job:set_share_target(NewJobShareTarget, NewJob),
            %% We stop accepting blocks until the {chain, {send_notify, Job}}
            %% event is processed. There might be some other new block chain
            %% events in the message queue of this process, if they were
            %% accepted it could result in multiple set_target notifications
            %% in a row.
            self() ! {chain, {send_notify, NewJob1}},
            State1 = State#state{accept_blocks = false},
            send_set_target_ntf(NewJobShareTarget, State1);
        _Other ->
            %% We keep the same target, so only notify notification is sent.
            NewJob1 = aestratum_job:set_share_target(OldJobShareTarget, NewJob),
            NewJobId = aestratum_job:id(NewJob1),
            BlockHash = aestratum_job:block_hash(NewJob1),
            BlockVersion = aestratum_job:block_version(NewJob1),
            %% TODO: compute if the client's job queueu should be cleaned.
            State1 = State#state{jobs = aestratum_job_queue:add(NewJob1, Jobs)},
            send_notify_ntf(NewJobId, BlockHash, BlockVersion, true, State1)
    end.

handle_chain_send_notify(Job, #state{accept_blocks = false, jobs = Jobs} = State) ->
...
send_set_target_ntf(ShareTarget, State) ->
    NtfMap = #{type => ntf, method => set_target,
               target => aestratum_target:to_bin(ShareTarget)},
